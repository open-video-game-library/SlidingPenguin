...
public class OutputDataManager : MonoBehaviour
{
    ...
    [System.Serializable]
 	...
 	// 試行が終わったときに呼び出す関数
    public void PostData(bool success, int fishNumber, string clearTime, float distance,
        float sensitivity, int limitedTime, float maximumSpeed, float acceleration, float friction)
     {
        ...
        data.MaximumSpeed = maximumSpeed; // 最高速度(詳細設定有効時に変更可能)
        data.Acceleration = acceleration; // 加速度(詳細設定有効時に変更可能)
        data.Friction = friction; // 摩擦(詳細設定有効時に変更可能)
